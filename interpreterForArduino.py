    ...
    try:
        ...
        if '101' in ardDecode: # Button -- Mode 
            ...
            if modePressed == 0:
                ...
                call(shlex.split('xbmc-send --action="ActivateWindow(music,sources://music/)"'))
        ...
        if '201' in ardDecode: # Button -- Power
            if modePressed == 1:
                call(shlex.split('xbmc-send --action="PlayerControl(Select)"'))
            else:
                call(shlex.split('xbmc-send --action="PlayerControl(Play)"'))

        if '211' in ardDecode: # Button -- Power Long
            if modePressed == 1:
                call(shlex.split('xbmc-send --action="Action(PartyMode)"'))
            else:
